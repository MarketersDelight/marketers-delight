<div id="md_popup_<?php echo esc_attr( $fields['id'] ); ?>" class="<?php echo esc_attr( $fields['classes'] . ' image-' . $fields['featured_image']['alignment']['center'] ); ?>"<?php echo md_style( array( 'bg_color' => $fields['bg_color'], 'border' => array( 7, 'solid', $fields['border_color'] ), 'color' => $fields['text_color'] ) ); ?>>
	<?php if ( empty( $fields['enable_custom_template'] ) ) : ?>
		<div class="popup-inner">
			<div class="popup-inner-bg popup-secondary-bg-color"<?php echo md_style( array( 'bg_color' => $fields['secondary_color'], 'bg_image' => $fields['bg_image'], 'bg_size' => $fields['bg_repeat'] ) ); ?>></div>
			<div class="popup-content block-single">
				<?php if ( ! empty( $fields['featured_image']['id'] ) ) : ?>
					<?php echo wp_get_attachment_image( $fields['featured_image']['id'], 'full', false, array(
						'class' => join( ' ', array( 'popup-image', $fields['featured_image']['classes'], $fields['featured_image']['alignment']['center'] ) ),
						'style' => $fields['featured_image']['style']['width'],
						'data-default-align' => 'aligncenter'
					) ); ?>
				<?php endif; ?>
				...
				<?php if ( ! empty( $fields['headline'] ) || is_customize_preview() ) : ?>
					<p class="popup-title med-title mb-half"<?php echo md_style( array( 'color' => $fields['headline_color'] ) ); ?>><?php echo md_text_field( $fields['headline'] ); ?></p>
				<?php endif; ?>
				<?php if ( ! empty( $fields['description'] ) || is_customize_preview() ) : ?>
					<div class="popup-text text-sec mb-single"><?php echo wpautop( $fields['description'] ); ?></div>
				<?php endif; ?>
				<?php if ( ! empty( $fields['button']['enable'] ) || ! empty( $fields['button_sec']['enable'] ) ) : ?>
					<div class="popup-buttons mb-single clear">
	...
	<?php endif; ?>
	<div class="md-popup-close md-popup-close-corner"<?php echo md_style( array( 'color' => $fields['close_color'] ) ); ?>>&times;</div>
</div>