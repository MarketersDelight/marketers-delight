        ...
        <div class="stream-text">
            <?php if ($title) : ?>
                <?php if (!is_singular() && !$has_titles) : ?>
                    <h1 class="stream-title small-title mb-half"><a
                                href="<?php the_permalink(); ?>"><?php echo $title; ?></a></h1>
...
<?php else : ?>
    <div class="stream-embed block-half">
        <div class="stream-embed-content clear">
            ...
            <?php endif; ?>
            <div class="stream-text">
                <p class="stream-title small-title mb-small"><a href="<?php echo $embed['link']; ?>"
                                                                title="<?php echo $embed['title']; ?>"><?php echo $embed['title']; ?></a>
                </p>
...
<?php if (get_post_type() !== 'stream_activity' && !is_singular() || (is_singular() && $c == 0)) : ?>
    <?php if ($has_thread) : ?>
        <p><a href="<?php echo get_permalink(); ?>#stream_<?php echo esc_attr($first_thread); ?>"
              class="stream-thread-text"><?php echo sprintf(__('Show thread (%s)', 'md'), count($thread)); ?></a></p>
    <?php endif; ?>
    <?php if (md_has('share')) {
        $share = new md_share;
