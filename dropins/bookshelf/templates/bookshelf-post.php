...
<div class="book"<?php echo $listing == 'grid' ? 'style="width: ' . round( 100 / $columns_count ) . '%;"' : ''; ?>>
	<?php if ( ! empty( $post['content'] ) ) : ?>
		<span class="book-thumb md-popup-trigger" data-popup="md_popup_bookshelf_<?php echo esc_attr( $post['post_id'] ); ?>">
			<?php echo get_the_post_thumbnail( $post['post_id'], 'md-book' ); ?>
		</span>
	<?php else : ?>
		<span class="book-thumb">
			<a href="<?php echo ! empty( $post['download_url'] ) ? esc_url( $post['download_url'] ) : $post['link']; ?>" target="_blank">
				<?php echo get_the_post_thumbnail( $post['post_id'], 'md-book' ); ?>
			</a>
		</span>
	<?php endif; ?>
...
<?php if ( $listing == 'excerpt' ) : ?>
	<div class="book-text block-half">
		...
		<div class="book-text-content block-half-bot">
			...
			<p>
				<?php if ( ! empty( $post['content'] ) ) : ?>
					<span class="book-review middot"><a href="<?php echo esc_url( $post['link'] ); ?>"><?php echo $post['string']; ?></a></span>
				<?php endif; ?>
