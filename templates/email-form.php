		...
		<?php if ( empty( $code ) ) : ?>
			...
			<form action="<?php echo md_email_action( $service, $list ); ?>" method="post" class="email-form <?php echo esc_attr( $form_classes ); ?>"<?php echo md_email_attrs( 'form', $fields ); ?>>
				...
				<?php if ( ! empty( $fields['email_input']['name'] ) ) : ?>
					<input type="text" class="form-input form-input-name" name="<?php echo md_email_input( 'name', $service ); ?>" placeholder="<?php echo esc_attr( $name_label ); ?>" required />
				<?php endif; ?>

				<input type="email" class="form-input form-input-email" name="<?php echo md_email_input( 'email', $service ); ?>" placeholder="<?php echo esc_attr( $email_label ); ?>" required />

				<button class="email-form-submit form-submit mb-half<?php echo trim( esc_attr( $submit_classes ) ); ?>"<?php echo $submit_style; ?><?php echo md_email_attrs( 'submit', $fields ); ?>><?php echo esc_attr( $submit_text ); ?></button>
			...
			<?php if ( $service == 'convertkit' ) : ?>
				<div id="ck_error_msg" style="display: none;">
					<p class="required"><i class="md-icon-cancel"></i> <?php echo __( 'There was an error submitting your subscription. Please try again.', 'md' ); ?></p>
				</div>
				<div id="ck_success_msg" style="display: none;">
					<p class="block-half" style="background-color: #22A340; border-radius: 3px; color: #fff;"><i class="md-icon-ok-circled"></i> <?php echo __( 'Thanks for joining! Check your email to complete your subscription', 'md' ); ?></p>
				</div>
			...
			<?php elseif ( $service == 'mailerlite' ) : ?>
				<div class="ml-form-successBody ml-block-success row-success" style="display:none">
					...
					<p class="block-half" style="background-color: #22A340; border-radius: 3px; color: #fff;"><i class="md-icon-ok-circled"></i> <?php echo __( 'Thank you for joining! Check your email to complete your subscription.', 'md' ); ?></p>
					</div>
				</div>
