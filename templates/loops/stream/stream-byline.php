<div class="byline">
	...
	<?php if ( ! empty( $embed_id ) && ! $is_embed ) :
		...
		$html_id = ! empty( $args['html_id'] ) && $post_id !== $args['html_id'] ? "#{$post_type}_" . $args['html_id'] : '';
		$short_name = get_the_author_meta( 'first_name', $post_author );
		if ( empty( $short_name ) )
			$short_name = get_the_author_meta( 'display_name', $post_author );
	...
	?>
		...
		<?php if ( get_post_type() == 'stream_activity' ) : ?>
			<?php echo md_icon( 'plus', array( 'classes' => 'mr-small' ) ); ?> <?php echo $short_name; ?> <?php echo sprintf( __( ' published a new %s', 'md' ), '<b>' . ( isset( $types[$post_type] ) ? $types[$post_type]['label'] : $post_type ) . '</b>' ); ?> <?php echo $label; ?>
		<?php else : ?>
			<?php echo md_icon( 'share', array( 'classes' => 'mr-small' ) ); ?> <?php echo $short_name; ?> <?php echo sprintf( __( ' shared a %s', 'md' ), '<b>' . ( isset( $types[$post_type] ) ? $types[$post_type]['label'] : $post_type ) . '</b>' ); ?> <?php echo $label; ?><a href="<?php echo get_permalink( $post_id ); ?><?php echo $html_id; ?>"><?php echo $date; ?></a>
		<?php endif; ?>
	...
	<?php if ( get_post_type() !== 'stream_activity' && current_user_can( 'edit_posts' ) ) : ?>
