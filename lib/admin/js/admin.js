	...
	window.MD = {
		...
		action: function() {
			$( document ).on( 'click', '.md-action', function( e ) {
				e.preventDefault();
				var trigger = $( this );
				if ( ! confirm( 'You are about to revert to the default MD font icons set. No files will be deleted, but your current icons\ndata will be removed and any custom icons may no longer show on your website. Do you wish to proceed?' ) )
					return;
				$.ajax({
					url: ajaxurl,
					type: 'POST',
