...
<?php if (has_nav_menu('main')) :
	...
	<div class="md-sep-small">
		...
		<div id="main_menu_options"
			 style="display: <?php echo empty($main_menu['remove']) ? 'block' : 'none'; ?>; margin-top: 10px;">
	...
	<div id="content_options" style="display: <?php echo empty($content['remove']) ? 'block' : 'none'; ?>;">
		...
		<?php if ($screen->base !== 'term') : ?>
			...
			<?php if ($screen->post_type != 'page') : ?>
				<div id="headline_options"
					 style="display: <?php echo empty($content['headline']) ? 'block' : 'none'; ?>;">
					<?php $this->fields->field('content', array(
							'type' => 'checkbox',
							'options' => array(
