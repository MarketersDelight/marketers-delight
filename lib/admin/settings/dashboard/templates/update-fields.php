	...
	<div class="md-widget-item md-clear">
		...
		<div class="md-updates">
			...
			?>
				<div class="md-update md-update-theme update-message notice inline notice-warning">
					<h4 class="md-widget-subtitle"><?php echo sprintf( __( 'Marketers Delight %s', 'md' ), $theme_version ); ?></h4>
					...
					?>
						<div class="md-update md-dropin">
							...
							<div class="md-dropin-content">
								<h4 class="md-widget-subtitle"><?php echo sprintf( __( '%s %2s', 'md' ), $dropin_name, $dropin_version ); ?></h4>
								<a href="<?php echo admin_url( wp_nonce_url( 'update.php?action=update-md-dropins&amp;dropin=' . urlencode( $dropin_path ), 'upgrade-dropin_' . $dropin_path ) ); ?>" class="md-update-button"  data-md-alert="<?php echo sprintf( __( 'NOTICE: You are about to upgrade to the latest version of %s. Any changes made directly to the dropin files in the /md-dropins/ directory will be overwritten with the latest files.', 'md' ), $dropin_name ); ?>"><?php echo sprintf( __( 'Upgrade now', 'md' ), $dropin_version ); ?> <i class="dashicons dashicons-update-alt"></i></a>
							</div>
						</div>
					<?php endforeach; ?>
				</div>
		...
		<div class="md-license-fields md-toggle<?php echo ( ! empty( $option['license']['status'] ) && $option['license']['status'] != 'valid' ? ' invalid open' : '' ); ?>">
			<?php if ( ! empty( $option['license']['status'] ) && $option['license']['status'] == 'valid' ) : ?>
				<span class="md-action md-action-check-updates" data-md-action="check-updates" data-md-canvas="#md_update"><?php echo __( 'Check for updates', 'md' ); ?> <i class="dashicons dashicons-update-alt"></i></span>
			<?php endif; ?>
