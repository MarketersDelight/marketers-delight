...
function md__register_required_plugins() {
	...
	$plugins = array(
		...
		array(
			'name'               => 'TGM Example Dropin', // The plugin name.
		...
		array(
			'name'         => 'Docs Dropin', // The plugin name.
			'slug'         => 'tgm-new-media-plugin-docs', // The plugin slug (typically the folder name).
		...
		array(
			'name'         => 'Optins Dropin', // The plugin name.
			'slug'         => 'tgm-new-media-plugin-optins', // The plugin slug (typically the folder name).
			'source'       => 'https://s3.amazonaws.com/tgm/tgm-new-media-plugin.zip', // The plugin source.
			'required'     => true, // If false, the plugin is only 'recommended' instead of required.
			'external_url' => 'https://github.com/thomasgriffin/New-Media-Image-Uploader', // If set, overrides default API URL and points to an external URL.
		),
		...
		array(
			'name'         => 'Testimonials Dropin', // The plugin name.
			'slug'         => 'tgm-new-media-plugin-testimonials', // The plugin slug (typically the folder name).
			'source'       => 'https://s3.amazonaws.com/tgm/tgm-new-media-plugin.zip', // The plugin source.
			'required'     => true, // If false, the plugin is only 'recommended' instead of required.
			'external_url' => 'https://github.com/thomasgriffin/New-Media-Image-Uploader', // If set, overrides default API URL and points to an external URL.
		),
		...
		array(
			'name'         => 'Generator Dropin', // The plugin name.
			'slug'         => 'tgm-new-media-plugin-gen', // The plugin slug (typically the folder name).
			'source'       => 'https://s3.amazonaws.com/tgm/tgm-new-media-plugin.zip', // The plugin source.
			'required'     => true, // If false, the plugin is only 'recommended' instead of required.
			'external_url' => 'https://github.com/thomasgriffin/New-Media-Image-Uploader', // If set, overrides default API URL and points to an external URL.
		),
		...
		// This is an example of how to include a plugin from the WordPress Dropin Repository.
		array(
			'name'      => 'BuddyPress',
			'slug'      => 'buddypress',
			'required'  => false,
...
function external_dropin_list() {
	...
	foreach( $response as $key => $value) {
		...
		$dropins[ $slug ] = [
			...
			'version'            => $value['name'], // E.g. 1.0.0. If set, the active plugin must be this version or higher. If the plugin version is higher than the plugin version installed, the user will be notified to update the plugin.
			'external_url'       => $value['package'], // If set, overrides default API URL and points to an external URL.
		];

		$_dropins[] = $dropins[ $slug ];

	}

	return $dropins;

	//var_dump( $_dropins ); die();
}
