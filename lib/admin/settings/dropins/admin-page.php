<div class="md-dropins md-content-wrap-med">
	<h2 class="md-title md-sep-small"><?php echo __('Drop-ins', 'md'); ?> &nbsp;<button id="md_upload_dropin_button"
																						class="button"><?php echo __('Add new', 'md'); ?></button>
	</h2>
	...
	<div class="md-dropins-list md-tabs md-sep-small">
		<div class="md-dropins-title">
			<h3>
				<a href="#" class="md-tab<?php echo md_get_dropins('installed', 'active') ? ' nav-tab-active' : ''; ?>"
				   data-md-tab="md-all"><?php echo sprintf(__('Installed <span>(%s)</span>', 'md'), count(md_get_dropins('installed'))); ?></a>
				<?php if (md_get_dropins('installed', 'active')) : ?>
					<a href="#" class="md-tab"
					   data-md-tab="dropin-enabled"><?php echo sprintf(__('Active <span>(%s)</span>', 'md'), count(md_get_dropins('installed', 'active'))); ?></a>
					<a href="#" class="md-tab"
					   data-md-tab="dropin-inactive"><?php echo sprintf(__('Inactive <span>(%s)</span>', 'md'), count(md_get_dropins('installed', 'inactive'))); ?></a>
				<?php endif; ?>
			</h3>
		</div>
				...
				<div class="md-dropin md-tab-content active md-all <?php echo $is_enabled ? 'dropin-enabled' : 'dropin-inactive'; ?>">
					<div class="columns-2 columns-10-90 columns-half">
						...
						<div class="md-dropin-content col col2">
							<h4 class="md-title"><a href="<?php echo esc_url($fields['dropin_url']); ?>"
													target="_blank"><?php echo esc_html($fields['name']); ?>
									<small><?php echo $fields['version']; ?></small></a></h4>
							<span class="md-delete md-action" data-md-action="delete-dropin"
								  data-md-dropin-id="<?php echo esc_attr($dropin); ?>"
								  data-md-alert="<?php echo sprintf(__("You are about to delete the %s Drop-in. All Drop-in files will be deleted, except from your child theme,\nand not all data will be saved. Do you want to proceed?", 'md'), $fields['name']); ?>"><i
										class="dashicons dashicons-no"></i> <?php echo __('Delete', 'md'); ?></span>
							<div class="md-dropin-controls">
								...
								<?php else : ?>
									<span class="md-dropin-plugin"><i
												class="dashicons dashicons-no"></i> <?php echo sprintf(__('Requires <b>%s</b> plugin', 'md'), $fields['plugin_name']); ?></span>
								<?php endif; ?>
							</div>
							...
							<p class="md-dropin-byline">
								<?php if ($is_enabled) : ?>
									<?php if (isset($fields['settings_url'])) : ?>
										<a href="<?php echo admin_url($fields['settings_url']); ?>"
										   class="button button-icon"><i
													class="dashicons dashicons-admin-generic"></i> <?php echo __('Settings', 'md'); ?>
										</a>
									<?php endif; ?>
									<a href="<?php echo esc_url($fields['dropin_url']); ?>" class="button"
									   target="_blank"><?php echo __('Get updates', 'md'); ?></a>
								<?php endif; ?>
	...
	<div class="md-dropins-list md-tabs md-sep">
		<div class="md-dropins-title">
			<h3>
				<a href="#" class="md-tab<?php echo $core_active_count ? ' nav-tab-active' : ''; ?>"
				   data-md-tab="md-all"><?php echo sprintf(__('Core <span>(%s)</span>', 'md'), $core_count); ?></a>
				<?php if ($core_active_count) : ?>
					<a href="#" class="md-tab"
					   data-md-tab="dropin-enabled"><?php echo sprintf(__('Active <span>(%s)</span>', 'md'), $core_active_count); ?></a>
					<a href="#" class="md-tab"
					   data-md-tab="dropin-inactive"><?php echo sprintf(__('Inactive <span>(%s)</span>', 'md'), ($core_count - $core_active_count)); ?></a>
				<?php endif; ?>
			</h3>
		</div>
				...
				<div class="columns-2 columns-10-90 columns-half">
					...
					<div class="md-dropin-content col col2">
						...
						<p class="md-dropin-byline">
							...
							<?php echo sprintf(__('<i>by</i> <a href="%s" target="_blank">%1s</a>', 'md'), $fields['author_url'], $fields['author']); ?></b>
							<?php if ($needs_plugin) : ?>
								<span class="md-dropin-plugin-required"><i
											class="dashicons dashicons-info"></i> <?php echo sprintf(__('Requires <b>%s</b> plugin', 'md'), $fields['plugin_name']); ?></span>
							<?php endif; ?>
						</p>
					</div>
				</div>
