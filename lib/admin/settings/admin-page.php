    ...
    <?php if (!empty($admin_pages[$page_id]['admin_header'])) : ?>
        <div class="md-header md-clear md-content-wrap-med">
            <div class="md-header-title">
                <h1><?php echo __('Marketers Delight', 'md'); ?> <a href="https://marketersdelight.com/changelog/"
                                                                    target="_blank"
                                                                    class="title-count theme-count"><?php echo MD_VERSION; ?></a>
                </h1>
                <?php settings_errors(); ?>
            </div>
            <div class="nav-tab-wrapper">
                <p class="md-links">
                    <a href="https://marketersdelight.com/downloads/"
                       target="_blank"><?php echo __('Account', 'md'); ?></a> &nbsp;&middot;&nbsp;
                    <a href="https://marketersdelight.com/docs/" target="_blank"><?php echo __('Docs', 'md'); ?></a>
                    &nbsp;&middot;&nbsp;
                    <a href="https://marketersdelight.com/affiliates/"
                       target="_blank"><?php echo __('Affiliates', 'md'); ?></a> &nbsp;&middot;&nbsp;
                ...
                <h2 class="md-header-nav md-clear">
                    <?php foreach ($admin_pages as $admin_page => $fields) :
                        ...
                        <a href="<?php echo admin_url("admin.php?page={$slug}"); ?>"
                           class="nav-tab<?php echo $page == $fields['id'] ? ' nav-tab-active' : ''; ?>"><?php echo esc_html($name); ?></a>
                    <?php endforeach; ?>
                </h2>
            </div>
                ...
                <div class="md-submenu">
                    <?php foreach ($admin_tabs as $admin_tab => $child) :
                        ...
                        if (isset($child['admin_tab_parent']) && $child['admin_tab_parent'] == $page)
                            $tab_url = '';
                        ?>
                        <a href="?page=<?php echo urlencode($page) . $tab_url; ?>"
                           class="md-submenu-item<?php echo $tab == $admin_tab ? ' md-submenu-active' : ''; ?>"
