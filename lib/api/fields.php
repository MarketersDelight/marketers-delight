...
class md_fields
{
    ...
    public function label($id, $args)
    {
        ...
        ?>
        <p class="md-label-wrap"><label for="<?php echo esc_attr($id); ?>"
                                        class="md-label<?php echo $classes; ?>"><?php echo $args['label']; ?></label>
        </p>
    <?php }
    ...
    public function text($name, $id, $option, $args)
    {
        ...
        <input type="<?php echo $type; ?>" name="<?php echo $name; ?>" id="<?php echo $id; ?>"
               value="<?php echo esc_attr(stripslashes($value)); ?>"<?php echo $placeholder; ?>
               class="regular-text<?php echo esc_attr($classes); ?><?php echo esc_attr($populate); ?>"<?php echo $readonly; ?><?php echo $style; ?><?php echo $disabled; ?> />
    <?php }
    ...
    public function textarea($name, $id, $option, $args)
    {
        ...
        ?>
        <textarea name="<?php echo $name; ?>" id="<?php echo $id; ?>" class="large-text"
                  rows="<?php echo $rows; ?>"><?php echo esc_attr(stripslashes($option)); ?></textarea>
    <?php }
    ...
    public function number($name, $id, $option, $args)
    {
        ...
        <input type="number" class="regular-text" name="<?php echo $name; ?>"
               id="<?php echo $id; ?>" value="<?php echo esc_attr($option); ?>"<?php echo $placeholder; ?> style="width: <?php echo $width; ?>px;"<?php echo $max; ?> /><?php echo $unit; ?>
    <?php }
    ...
    public function code($name, $id, $option, $args)
    {
        ...
        <div class="md-code-editor">
            <textarea name="<?php echo $name; ?>" id="<?php echo $id; ?>" class="large-text"
                      rows="<?php echo esc_attr($rows); ?>"><?php echo stripslashes($option); ?></textarea>
        </div>
    ...
    public function url($name, $id, $option, $args)
    {
        ...
        <input type="text" name="<?php echo $name; ?>" id="<?php echo $id; ?>"
               value="<?php echo esc_attr(esc_url($option)); ?>"
               class="regular-text"<?php echo $placeholder; ?><?php echo $disabled; ?> />
    <?php }
    ...
    public function hidden($name, $id, $option, $args)
    {
        ...
        <input type="hidden" name="<?php echo $name; ?>" id="<?php echo $id; ?>"
               class="regular-text<?php echo $classes; ?>" value="<?php echo esc_attr(stripslashes($option)); ?>"/>
    <?php }
    ...
    { ?>
        <div class="md-checkboxes<?php echo isset($args['multi']) ? ' md-multi-checkbox' : ''; ?>">
            <?php foreach ($args['options'] as $val => $label) :
                ...
                ?>
                <p class="md-checkbox">
                    <input type="checkbox" name="<?php echo $nameval; ?>" id="<?php echo $idval; ?>"
                           value="1"<?php echo checked($check); ?> />
    ...
    { ?>
        ...
        <label for="<?php echo $idval; ?>" class="md-radio<?php echo(is_array($label) ? ' md-radio-image' : ''); ?>">
            <input type="radio" name="<?php echo $name; ?>" id="<?php echo $idval; ?>" class="md-radio-check"
                   value="<?php echo esc_attr($val); ?>"<?php echo checked($option, $val); ?> />
            <span class="md-radio-label"<?php echo $image; ?>><span
                        class="md-radio-text"><?php echo $text; ?></span></span>
        </label>
    <?php endforeach; ?>
    <?php }
    ...
    public function range($name, $id, $option, $args)
    {
        ...
        ?>
        <p class="md-range">
            <input type="range" class="md-range-field" value="<?php echo esc_attr($option); ?>"
                   min="<?php echo $min; ?>" max="<?php echo $max; ?>"/>
            <input name="<?php echo $name; ?>" id="<?php echo $id; ?>" class="md-range-number" type="number"
                   value="<?php echo esc_attr($option); ?>" placeholder="<?php echo $placeholder; ?>"
                   style="width: 65px;"/> <?php echo esc_html($unit); ?>
    ...
    public function color($name, $id, $option, $args)
    {
        ...
        ?>
        <input type="text" name="<?php echo $name; ?>" id="<?php echo $id; ?>" value="<?php echo esc_attr($option); ?>"
               placeholder="<?php echo $placeholder; ?>" class="<?php echo esc_attr($class); ?>"<?php echo $alpha; ?> />
        ...
        <div class="md-upload md-upload-<?php echo $type; ?><?php echo !empty($upload_url) ? ' has-upload' : ''; ?>">
            <div class="md-uploader">
                ...
                <div class="md-upload-controls">
                    ...
                    <input type="url" class="md-upload-url regular-text" name="<?php echo $name; ?>[url]"
                           id="<?php echo "{$id}_url"; ?>" value="<?php echo esc_attr($upload_url); ?>"
                           placeholder="https://">
                    <input type="hidden" class="md-upload-id regular-text" name="<?php echo $name; ?>[id]"
                           id="<?php echo "{$id}_id"; ?>" value="<?php echo esc_attr($upload_id); ?>" placeholder="">
                    <?php if ($upload_id) : ?>
    ...
    <?php elseif ($type == 'file') :
        ...
        <div class="md-file-upload">
            <div class="md-file-upload-field">
                <input type="file" name="<?php echo $name; ?>[url]"
                       id="<?php echo esc_attr("{$id}_file"); ?>"<?php echo $accept; ?> />
                <span class="md-loading md-file-uploading"><i class="dashicons dashicons-update-alt"></i></span>
                <span class="md-tooltip md-file-upload-success"><i
                            class="dashicons dashicons-yes"></i> <?php echo esc_html($success_text); ?></span>
            </div>
        </div>
    ...
    public function group($name, $id, $option, $args)
    {
        ...
        <div id="md_group_<?php echo esc_attr("{$this->_id}_" . $args['field']); ?>"
             class="md-groups md-group-<?php echo $style; ?>">
                ...
                <div class="md-group<?php echo ($valid) . ("$group" == $var ? ' empty' : '') . ($style == 'boxes' ? ' md-widget md-toggle' : ''); ?>">
                    <div class="md-group-controls<?php echo($style == 'boxes' ? ' md-widget-title' : ''); ?>">
                        ...
                        <span class="md-group-controls-inner">
							<span class="md-delete dashicons dashicons-no"
                                  title="<?php echo __('Delete', 'md'); ?>"></span>
							<span class="md-reorder dashicons dashicons-menu"
                                  title="<?php echo __('Reorder', 'md'); ?>"></span>
						</span>
                    </div>
    ...
    public function clone_button($field, $args = null)
    {
        ...
        ?>
        <span class="md-clone-add button<?php echo esc_attr($classes); ?>"
              data-clone-group="<?php echo esc_attr("{$this->_id}_" . $field); ?>"><?php echo $label; ?></span>
    <?php }
    ...
    public function media($name, $id, $option, $args)
    {
        ...
        <div class="md-media">
            <div class="md-media-columns columns-2 columns-30-70">
                <div class="col col1">
                    <div class="col-inner">
                        <p><img class="md-media-preview-image md-media-add" src="<?php echo $src; ?>"
                                alt="<?php _e('Preview image', 'md'); ?>"/></p>
                    </div>
                </div>
                <div class="col col2">
                    <div class="col-inner">
                        <p>
                            <label class="md-label"
                                   for="<?php echo $name; ?>[url]"><?php echo __('Image Link', 'md'); ?></label>
                            <input type="url" class="md-media-url regular-text" name="<?php echo $name; ?>"
                                   id="<?php echo $id; ?>" value="<?php echo esc_attr($option); ?>"
                                   placeholder="<?php _e('http://', 'md'); ?>">
                        </p>
            ...
            <div class="md-media-buttons">
                <input type="button" class="md-media-add button" value="<?php _e('Add Image', 'md'); ?>"
                       style="display: <?php echo empty($option) ? 'inline-block' : 'none'; ?>;"/>
                <input type="button" class="md-media-remove button" value="<?php _e('Remove Image', 'md'); ?>"
                       style="display: <?php echo !empty($option) ? 'inline-block' : 'none'; ?>;"/>
            </div>
        </div>
        <?php wp_enqueue_media(); ?>
    ...
    public function save($label = null, $args = null)
    {
        ...
        ?>
        <input type="submit" name="submit" id="submit" class="button button-primary md-button"
    ...
    { ?>
        <div class="md-devices alignright">
			<span id="desktop" class="md-device desktop active"
                  title="<?php echo __('Switch to Desktop controls', 'md'); ?>">
