...
class Dropin_Installer_Skin extends WP_Upgrader_Skin {
	...
	public function after() {
		// Check if the dropin can be overwritten and output the HTML.
		...
		/**
		 * Filters the list of action links available following a single dropin installation.
		 *
		 * @since 2.7.0
		 *
		 * @param string[] $install_actions Array of dropin action links.
	 ...
	 * @since 5.5.0
	 *
	 * @return bool Whether the dropin can be overwritten and HTML was outputted.
	 */
	private function do_overwrite() {
		...
		$is_same_dropin = true; // Let's consider only these rows.
		...
		/**
		 * Filters the compare table output for overwriting a dropin package on upload.
		 *
		 * @since 5.5.0
		 *
		 * @param string $table               The output table with Name, Version, Author, RequiresWP, and RequiresPHP info.
		 * @param array  $current_dropin_data Array with current dropin data.
		 * @param array  $new_dropin_data     Array with uploaded dropin data.
		 */
		...
		if ( ! is_wp_version_compatible( $requires_wp ) ) {
			$error = sprintf(
				...
				__( 'Your WordPress version is %1$s, however the uploaded dropin requires %2$s.' ),
		...
		if ( $can_update ) {
			if ( $this->is_downgrading ) {
				$warning = sprintf(
					...
					__( 'You are uploading an older version of a current dropin. You can continue to install the older version, but be sure to <a href="%s">back up your database and files</a> first.' ),
					__( 'https://wordpress.org/support/article/wordpress-backups/' )
				);
			} else {
				$warning = sprintf(
					/* translators: %s: Documentation URL. */
					__( 'You are updating a dropin. Be sure to <a href="%s">back up your database and files</a> first.' ),
					__( 'https://wordpress.org/support/article/wordpress-backups/' )
				);
			}
		...
		$install_actions['dropins_page'] = sprintf(
			...
			wp_nonce_url( $cancel_url, 'dropin-upload-cancel-overwrite' ),
			__( 'Cancel and go back' )
		);

		/**
		 * Filters the list of action links available following a single dropin installation failure
		 * when overwriting is allowed.
		 *
		 * @since 5.5.0
		 *
		 * @param string[] $install_actions Array of dropin action links.
		 * @param object   $api             Object containing WordPress.org API dropin data.
		 * @param array    $new_dropin_data Array with uploaded dropin data.
		 */
